<template>
    <div class="recharge-wrapper">
        <p class="d-recharge-message">{{content.title}} <span>剩余{{content.surplus_number}}次</span></p>
        <el-row :gutter="20">
            <el-col :span="8" v-for="(item, index) of content.recharge_result" :key="index">
        ...
        <div class="d-recharge-result">
            ...
            <p class="d-recharge-result-message">¥<span>{{content.result_item}}</span></p>
            <div class="d-recharge-result-img">
                <img :src="content.qr_code">
            </div>
            <p class="d-recharge-result-desc">请在<span>{{content.count_down}}</span>内完成支付</p>
        </div>
