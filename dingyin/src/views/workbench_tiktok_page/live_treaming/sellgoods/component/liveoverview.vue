<template>
    <div class="table_box">
        ...
        <el-table v-infinite-scroll="load" infinite-scroll-disabled="disabled" :data="tableData" style="width: 100%">
            ...
            <el-table-column min-width="140px" align="center" label="商品">
                <template slot-scope="scope">
                    <div class="columntow">
                        <div class="userImg">
                            <el-image :src="scope.row.towimgurl"></el-image>
            ...
            <el-table-column align="center" label="商品成交价范围">
                <template slot-scope="scope">
                    <span>{{scope.row.threetitle}}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="直播佣金比例">
                <template slot-scope="scope">
                    <span>{{scope.row.fourtitle}}</span>
                </template>
            </el-table-column>
            <el-table-column sortable align="center" label="关联直播达人" prop="fivetitle">
                <template slot-scope="scope">
                    <span>{{scope.row.fivetitle}}</span>
                </template>
            </el-table-column>
            <el-table-column sortable align="center" label="关联直播" prop="sixtitle">
                <template slot-scope="scope">
                    <span>{{scope.row.sixtitle}}</span>
                </template>
            </el-table-column>
            <el-table-column sortable align="center" label="直播带货销量" prop="seventitle">
                <template slot-scope="scope">
                    <span>{{scope.row.seventitle}}</span>
                </template>
            </el-table-column>
            <el-table-column sortable align="center" label="直播带货销售额" prop="eighttitle">
                <template slot-scope="scope">
...
import classificationdata from '@/components/page/classification_data'
    export default {
        ...
        methods: {
            ...
            goRealdetailed() {
                this.$router.replace('/realdetailed');
            },
            //主要按钮前往详情大屏
            goRealdetailscreen() {
            ...
            getMessage(){
                return this.$http({
                    url: "/segoods_commodity_table.json",
                    method: "get",
                }).then(({
                    data
                }) => {
                    ...
                    // 数据分类 数据
                    this.classification_data = data.classification_data
                }).catch(err => {
